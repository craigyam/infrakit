...
func run(t *testing.T, resourceType, properties string) {
	...
	for key, resources := range m["resource"].(map[string]interface{}) {
		...
		case FSSoftLayer:
			_, storageName, _, err := FindStorage(&tformat)
			require.NoError(t, err)
			// Storage resources should be renamed to include the id
			require.Equal(t, string(storageName), fmt.Sprintf("%s-%s", resourceName, string(*id2)))
		case VMSoftLayer:
